  ...
  return (
    <li key={id} className="overflow-hidden bg-stone-50">
      <div className="flex items-end gap-x-4 border-b border-stone-900/5 relative h-[200px]">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger className="absolute right-2 top-2 z-10 block text-stone-50 hover:text-stone-100 bg-stone-50/30 rounded-full p-1 hover:bg-stone-50/50 transition-all cursor-pointer">
            <span className="absolute -inset-2.5" />
            <span className="sr-only">Open options</span>
            <EllipsisVerticalIcon aria-hidden="true" className="size-5" />
          </DropdownMenuTrigger>
          <DropdownMenuContent className="absolute right-0 z-10 mt-0.5 px-0 rounded-none w-32 origin-top-right bg-white py-2 shadow-lg ring-1 ring-stone-900/5 transition focus:outline-hidden data-closed:scale-95 data-closed:transform data-closed:opacity-0 data-enter:duration-100 data-enter:ease-out data-leave:duration-75 data-leave:ease-in">
            <DropdownMenuItem className="px-3 py-1 rounded-none cursor-pointer">
              <a
                href="#"
                className="block text-sm/6 text-stone-900 data-focus:bg-stone-50 data-focus:outline-hidden"
              >
